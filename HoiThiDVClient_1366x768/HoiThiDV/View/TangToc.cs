...
namespace HoiThiDV.View
{
    public partial class TangToc : UserControl
    {
        ...
        public void LoadQuestion(DataServer data)
        {
            ...
            R2_Question.Text = "HÀNG NGANG SỐ " + data.QuestionNum + " (" + data.InitOchu[data.QuestionNum - 1].Replace("$", "").Length + " KÝ TỰ)";
        }
        ...
        public void InitTangToc(DataServer data)
        {
            ...
            R2_Question.Text = "";
            R2_TeamAnswer.Text = "Đáp án của bạn: ";
            R2_TxtAnswer.Text = "";
                ...
                for (int dong = 1; dong <= data.InitOchu.Count; dong++)
                {
                    ...
                    for (int cot = 1; cot <= 29; cot++)
                    {
                        if (data.InitOchu[dong - 1][cot - 1].Equals('$'))
                        {
                            ...
                            //Control comp100 = (Control)this.Controls.Find("R2_D" + dong + "_C100", true).FirstOrDefault();
                            if (InvokeRequired)
                            {
                                ...
                                //comp100.Invoke((MethodInvoker)delegate
                                //{
                                //    //comp100.Visible = false;
                                //});
                            }
                            else
                            {
        ...
        {
            if (!string.IsNullOrWhiteSpace(R2_TxtAnswer.Text.Trim()))
            {
                ...
                this.R2_TeamAnswer.Text = "Đáp án của bạn: " + txt;
                R2_TxtAnswer.Text = "";
                CompDisableEnable("R2_TxtAnswer", false);
                ...
                isAnswer = true;
                // Send to server
                this.ctrl.SendAnswer(answer, timeElapsed.TotalSeconds.ToString("0.00000"), 2);
            }
